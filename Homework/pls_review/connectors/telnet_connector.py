...
class TelnetConnector:
    ...
    def _initial_conf_ios(self):
        ...
        out = self.execute('crypto key generate rsa modulus 2048', prompt=[r'\(config\)#', r'replace them\?'])
        ...
        self.execute(f'username {username} privilege 15 password {password}', prompt=[r'\(config\)#'])
